...
import (
	...
	mock_payment "github.com/NStegura/saga/products/mocks/app/consumers/handlers/payment"
)
...
func TestIncomeHandler_ConsumeClaim_Success(t *testing.T) {
	...
	mockProduct := mock_payment.NewMockProduct(ctrl)
	mockCache := mock_payment.NewMockCache(ctrl)
	logger := logrus.New()

	handler := New(mockProduct, mockCache, logger)
	...
	mockProduct.EXPECT().UpdateReserveStatus(context.Background(), message.OrderID, true).Return(nil).Times(1)
...
func TestIncomeHandler_ConsumeClaim_UpdateReserveStatusError(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockCache.EXPECT().Get(context.Background(), message.IKey).Return(redis.ErrCacheMiss).Times(1)
	mockProduct.EXPECT().UpdateReserveStatus(
		context.Background(), message.OrderID, true,
	).Return(errors.New("update reserve status error")).Times(1)
